...
Simplified version with standard generation only (no mixture of tokens)
"""
import os
import json
import torch
...
def eval_on_test_set(
    ...
    Args:
        ...
        env_name: Optional environment name for logging
    ...
    print(f"Running evaluation on test set{' for ' + env_name if env_name else ''}...")
    ...
    log_file = os.path.join(args.output_dir, f'eval_metrics_{round_num}{env_suffix}.txt')
    test_loader.reset()
    
    with open(log_file, 'w') as f:
        # Run through test set
        for question, answer, entry in tqdm(test_loader, desc="Evaluating on test set"):
            # Generate completions using standard generation
    ...
    # Save metrics
    metrics_path = os.path.join(args.output_dir,f'eval_metrics_{round_num}{env_suffix}.json')
    with open(metrics_path, 'w') as f:
        json.dump({**avg_scores, 'accuracy': accuracy, 'environment': env_name or args.dataset}, f, indent=4)

    if args.verbose:
        print(f"\nEvaluation Results{' for ' + env_name if env_name else ''}:")
        print("-" * 20)
...
def generate_completions(
    ...
    """
    Generate multiple completion sequences for a given prompt using standard generation.
...
def score_completions(
    ...
    Args:
        ...
        args: Training arguments
        entry: Optional dataset entry for additional context
...
def parse_args():
    parser = argparse.ArgumentParser(description="GRPO training arguments (simplified - standard generation only)")
    ...
    parser.add_argument("--eval_environments", type=str, nargs='+', default=None, help="List of evaluation environments (e.g., gsm8k math500 reasoning_gym.acre). If not specified, uses --dataset")

    # Output and logging
    parser.add_argument("--output_dir", type=str, default="output_single", help="Directory to save outputs")
...
if __name__ == "__main__":
    ...
    ## Set up multiple evaluation environments if specified
    if args.eval_environments:
        eval_envs = {}
        for env_name in args.eval_environments:
            try:
                _, env_test_loader = rldatasets.get_dataloaders(env_name)
                env_eval_class = evaluator.get_evaluator(env_name)
                eval_envs[env_name] = {
                    'test_loader': env_test_loader,
                    'eval_class': env_eval_class
                }
                print(f"Loaded evaluation environment: {env_name}")
            except Exception as e:
                print(f"Warning: Could not load evaluation environment {env_name}: {e}")
    else:
        # If no eval environments specified, use the training dataset
        eval_envs = {
            args.dataset: {
    ...
    print("\n" + "="*80)
    print("Starting GRPO Training (Standard Generation Only)")
    print("="*80)
    print(f"Model: {args.model_name}")
    print(f"Training Dataset: {args.dataset}")
    print(f"Evaluation Environments: {list(eval_envs.keys())}")
    print(f"Number of chains: {args.num_chains}")
    print(f"Total iterations: {args.num_train_iters}")
    ...
    for round_num in tqdm(range(args.num_train_iters), desc="Training Progress"):
        ...
        if (round_num) % args.eval_iterations == 0:
            ...
            all_env_metrics = {}
            for env_name, env_config in eval_envs.items():
                eval_metrics, eval_accuracy = eval_on_test_set(
                    model=model,
                    tokenizer=tokenizer, 
                    test_loader=env_config['test_loader'],
                    eval_class=env_config['eval_class'],
                    device=device,
                    args=args,
                    round_num=round_num,
                    env_name=env_name
                ...
                all_env_metrics[env_name] = {
                    'metrics': eval_metrics,
                    'accuracy': eval_accuracy
                }
                
                # Save metrics to eval log dir (environment-specific)
                env_suffix = f"_{env_name}"
                metrics_path = os.path.join(eval_log_dir, f'metrics_{round_num}{env_suffix}.json')
                with open(metrics_path, 'w') as f:
                    json.dump({
            ...
            # Save combined metrics for all environments
            combined_metrics_path = os.path.join(eval_log_dir, f'metrics_{round_num}_all_envs.json')
            with open(combined_metrics_path, 'w') as f:
                json.dump(all_env_metrics, f, indent=4)
        ...
        total_loss, train_metrics = grpo_loss(
            ...
            eval_class, device, round_num, train_log_dir, args, entry
        )
